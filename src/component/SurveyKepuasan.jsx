  ...
  return (
    <div className="w-screen h-screen fixed bg-black top-0 left-0 bg-opacity-60 flex justify-center items-center z-50 text-black">
      <div className="bg-white relative w-3/4 lg:w-1/4 p-5 rounded-md flex flex-col items-center gap-2">
        <AiOutlineClose
          className="absolute right-4 top-5 w-5 h-6 cursor-pointer text-red-400"
          onClick={() => setOpenRating(false)}
        />
        <div className="font-bold text-center">Survey Kepuasan</div>
        <div className="flex flex-col gap-2">
          ...
          <div className="flex justify-between w-full text-xs">
            <div className="ml-[-18px]">Tidak Puas</div>
            <div className="mr-[-18px]">Sangat Puas</div>
          </div>
        ...
        {survey && (
          <div
            className={` p-2 mt-3 w-full rounded-md flex justify-center text-white text-center ${
              rating !== 0
          ...
          >
            {isLoading ? (
              <img src={Loading} className="w-5 h-5" alt="loading" />
            ) : (
              "Beri Rating"
            )}
          </div>
